...
class Server(object):
    ...
    *  ``{'op': 'add', 'x': 10, 'y': 20}``
    ...
    def handle_comm(self, comm, shutting_down=shutting_down):
        ...
        # print("Connection from {} to {}".format(address, type(self).__name__))
        self._comms[comm] = op
        try:
            while True:
                try:
                    msg = yield comm.read()
                    # print("Message from {}: {}", address, msg)
                ...
                try:
                    ...
                    try:
                        #if handler.__name__ != "heartbeat_worker":
                           #print("handler.__name__: ", handler.__name__)
                           #print("msg: ", msg)                        
