...
def dumps(msg, serializers=None, on_error="message", context=None, debug_print = False):
    ...
    try:
        ...
        if isinstance(msg, (list, dict)):
            ...
            if debug_print:
                print("isinstance(msg, (list, dict)) == TRUE")
                print("msg: ", msg)
                print("data: ", data)
                print("bytestrings: ", bytestrings)
        else:
            if debug_print:
                print("isinstance(msg, (list, dict)) == FALSE")
                print("msg: ", msg)
        small_header, small_payload = dumps_msgpack(msg)

        if not data:  # fast path without serialized data
            if debug_print:
                print("Taking the fast path -- no serialized data.")
