    ...
    * **coroutines:** ``[Futures]``:
    ...
    def __init__(
        ...
        **kwargs
        ...
        if (use_local_proxy):
            self.start_local_kv_proxy(local_proxy_path = local_proxy_path, aws_region = aws_region, proxy_address = proxy_address)
        else:
            self.local_kv_proxy_proc = None 
        ...
        if dashboard_address is not None:
            try:
                from wukong.bokeh.scheduler import BokehScheduler
            except ImportError:
    ...
    def start_local_kv_proxy(self, local_proxy_path = None, aws_region = None, proxy_address = None):
        """
        Start the KV Store Proxy as a separate Python process locally on this VM.
        """
        print("Automatically starting the KV Store Proxy locally.")
        self.local_kv_proxy_proc = subprocess.Popen(["python3", local_proxy_path, "--region", aws_region, "--redis", proxy_address])
    ...
    def close(self, comm=None, fast=False, close_workers=False):
        ...
        if self.local_kv_proxy_proc is not None:
            self.local_kv_proxy_proc.terminate()
...
#
    ...
    def get_task_stream(self, comm=None, start=None, stop=None, count=None):
        from wukong.diagnostics.task_stream import TaskStreamPlugin
